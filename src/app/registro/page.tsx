...
export default function RegistroPage() {
  ...
  useEffect(() => {
    if (!isStripeEnabled) router.replace('/login');
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!acceptedTerms) { showError('Debes aceptar los términos y la política de privacidad'); return; }
    setIsLoading(true);
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${window.location.origin}/auth/callback?mode=signup&next=/panel/bienvenida`,
      },
    });
    setIsLoading(false);
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/auth/callback?mode=signup&next=/panel/bienvenida` },
    });
    ...
    <ThemedPage className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative" style={{ backgroundColor: 'var(--l-bg)' }}>
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-light mt-4" style={{ color: 'var(--l-text)' }}>Crea tu cuenta gratis</h2>
          <p className="mt-2" style={{ color: 'var(--l-text-muted)' }}>25 facturas de prueba sin compromiso</p>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--l-card)', border: '1px solid var(--l-card-border)' }} className="p-8">
          ...
          <div className="mb-6 relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2" style={{ backgroundColor: 'var(--l-card)', color: 'var(--l-text-muted)' }}>O regístrate con</span>
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                <input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" required minLength={6}
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium mb-2" style={{ color: 'var(--l-text-secondary)' }}>Confirmar contraseña</label>
              <input id="confirmPassword" name="confirmPassword" type={showPassword ? 'text' : 'password'} autoComplete="new-password" required minLength={6}
                value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
                className="block w-full px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
                style={{ backgroundColor: 'var(--l-bg)', border: `1px solid ${passwordMismatch ? '#ef4444' : 'var(--l-card-border)'}`, color: 'var(--l-text)' }}
                placeholder="Repite tu contraseña" />
              {passwordMismatch && <p className="mt-1 text-sm text-red-500">Las contraseñas no coinciden</p>}
            </div>

            <div className="flex items-start">
              <input id="terms" type="checkbox" checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)}
                className="mt-1 h-4 w-4 text-primary focus:ring-primary" style={{ backgroundColor: 'var(--l-bg)', borderColor: 'var(--l-card-border)' }} />
              <label htmlFor="terms" className="ml-2 text-sm" style={{ color: 'var(--l-text-secondary)' }}>
                Acepto los{' '}
                <Link href="/terminos" target="_blank" className="text-primary hover:text-primary-hover underline">Términos de Servicio</Link>
                {' '}y la{' '}
                <Link href="/privacidad" target="_blank" className="text-primary hover:text-primary-hover underline">Política de Privacidad</Link>
              </label>
            </div>
        ...
        <p className="text-center text-sm" style={{ color: 'var(--l-text-muted)' }}>
          ¿Ya tienes cuenta?{' '}
          <Link href="/login" className="font-medium text-primary hover:text-primary-hover transition-colors">Inicia sesión</Link>
        </p>
      </div>
    </ThemedPage>
  );
