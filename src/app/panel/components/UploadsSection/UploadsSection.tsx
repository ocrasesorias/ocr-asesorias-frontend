  ...
  return (
    <div className="bg-[var(--l-card,#ffffff)] rounded-none shadow-sm border border-[var(--l-card-border,#e5e7eb)] overflow-hidden">
      ...
      {deletableSubidas.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mb-3 py-2 px-3 bg-[var(--l-bg,#f9fafb)] border border-[var(--l-card-border,#e5e7eb)] rounded-none">
          <label className="flex items-center gap-2 text-sm text-foreground-secondary cursor-pointer whitespace-nowrap">
            <input
              type="checkbox"
              checked={allSelected}
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button
              ...
              aria-label={selectedIds.size > 0 ? `Exportar ${selectedIds.size}` : 'Exportar'}
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-white bg-primary hover:bg-primary-hover rounded-none transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
            >
              ...
              {isBulkExporting ? '…' : selectedIds.size > 0 ? `Exportar ${selectedIds.size}` : 'Exportar'}
            </button>
            <button
              type="button"
              ...
              aria-label={selectedIds.size > 0 ? `Eliminar ${selectedIds.size}` : 'Eliminar'}
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 rounded-none transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
